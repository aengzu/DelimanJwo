...
def create_access_token(id: str, user_id: int, expires_delta: timedelta):
    encode = {'sub': id, 'id': user_id}
...
async def create_user(create_user_request: CreateUserRequest = Body(...), db: Session = Depends(get_db)):
    ...
    token = create_access_token(user.id, user.user_id, timedelta(minutes=120))
...
async def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    if not user:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='Could not validate user')
    token = create_access_token(user.id, user.user_id, timedelta(minutes=20))
    return {'access_token': token, 'token_type': 'bearer'}
